...
{
    DIRTY = -2,         // dirty page
    CLEAR = -1         // invalid page(Haven't write data：0XFFFFFFFF)
};
...
int dirty_block_table[PHYSICAL_NAND_NUM] = {0}; // Record the number of dirty page in every block to find the minimin GC
...
static unsigned int get_next_pca()
{
	...
	printf("[SYSTEM] call get_next_pca\n");
...
static int ftl_write(const char* buf, size_t lba_range, size_t lba)
{
    ...
    if (nand_write(buf, pca.pca) > 0)
    {
        ...
        if(L2P[lba] != CLEAR) //
        {
            ...
            dirty_block_table[outdate_pca.fields.block]++;
...
static unsigned int ftl_gc()
{
    ...
    {
        if(dirty_block_table[i] > max)
        {
            max = dirty_block_table[i];
            max_idx = i;
        }
    }
    if(max == 0)
...
{
/*  TODO (Done): only basic write case, need to consider other cases */
	printf("[SYSTEM] call ssd_do_write\n");
